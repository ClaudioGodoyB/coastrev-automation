...
def download_attachments(subject_keyword, save_folder, folder_date):
    # Updated folder structure: "Extract YYYY-MM-DD\RRM"
    folder_name = f"Extract {folder_date}\\RRM"
    ...
    if os.path.exists(full_folder_path):
        ...
        for message in messages:
            if message.SenderEmailAddress == 'no-reply@hotelkeyapp.com':  # Check if the sender matches
                if folder_date in message.ReceivedTime.strftime('%Y-%m-%d') and subject_keyword in message.Subject:
                    ...
                    break  # Stop after the latest relevant email is processed
        else:
            print(f"No email found for {folder_date} from 'no-reply@hotelkeyapp.com' with subject containing '{subject_keyword}' in folder: '{folder_name}'.")
    else:
...
if __name__ == "__main__":
    subject_keyword = "RRM Daily"  # Change this to your desired subject keyword
