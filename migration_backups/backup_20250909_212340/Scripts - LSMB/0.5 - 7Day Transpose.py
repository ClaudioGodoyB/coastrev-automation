...
destination_file = r"C:\Users\johnj\Desktop\CoastRev\Reporting\Daily Templates\LSMB.xlsx"
...
# Construct the folder path for the 7 days ago extract, including the 'LSMB' subfolder
source_folder = os.path.join(base_source_folder, f"Extract {seven_days_ago.strftime('%Y-%m-%d')}", "LSMB")
...
if not source_file:
    print(f"No source file found for {eight_days_ago.strftime('%d-%b-%Y')} in the {seven_days_ago.strftime('%Y-%m-%d')} 'LSMB' subfolder.")
else:
    try:
