#!/usr/bin/env python3
"""
MSI Email Download - VM Compatible Version  
Property: MSI 
Subject: 'MSI Daily Report'
Type: outlook
"""
import os
import sys
...
# MSI CONFIGURATION
PROPERTY_CODE = "MSI"
SUBJECT_KEYWORDS = "MSI Daily Report"
EMAIL_TYPE = "outlook"

def download_msi_emails():
    """Download MSI property emails using Gmail API"""
    try:
        print(f"🏨 Processing emails for MSI")
        ...
        if success:
            print(f"✅ Successfully processed MSI emails")
        else:
            print(f"⚠️ No MSI emails found")
            
        return success
        
    except Exception as e:
        print(f"❌ An error occurred processing MSI emails: {e}")
        return False

def main():
    """Main execution function"""
    print("MSI Email Download Script - Gmail API Version")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*60)
    
    success = download_msi_emails()
    
    print("="*60) 
    if success:
        print("✅ MSI email processing completed successfully")
    else:
        print("❌ MSI email processing failed or found no emails")
