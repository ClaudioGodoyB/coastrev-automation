#!/usr/bin/env python3
"""
LSMB Email Download - VM Compatible Version  
Property: LSMB 
Subject: 'LSMB Daily Report'
Type: outlook
"""
import os
import sys
...
# LSMB CONFIGURATION
PROPERTY_CODE = "LSMB"
SUBJECT_KEYWORDS = "LSMB Daily Report"
EMAIL_TYPE = "outlook"

def download_lsmb_emails():
    """Download LSMB property emails using Gmail API"""
    try:
        print(f"🏨 Processing emails for LSMB")
        ...
        if success:
            print(f"✅ Successfully processed LSMB emails")
        else:
            print(f"⚠️ No LSMB emails found")
            
        return success
        
    except Exception as e:
        print(f"❌ An error occurred processing LSMB emails: {e}")
        return False

def main():
    """Main execution function"""
    print("LSMB Email Download Script - Gmail API Version")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*60)
    
    success = download_lsmb_emails()
    
    print("="*60) 
    if success:
        print("✅ LSMB email processing completed successfully")
    else:
        print("❌ LSMB email processing failed or found no emails")
