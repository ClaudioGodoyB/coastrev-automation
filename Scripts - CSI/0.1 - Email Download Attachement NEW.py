#!/usr/bin/env python3
"""
CSI Email Download - VM Compatible Version  
Property: CSI 
Subject: 'CSI Daily Report'
Type: outlook
"""
import os
import sys
...
# CSI CONFIGURATION
PROPERTY_CODE = "CSI"
SUBJECT_KEYWORDS = "CSI Daily Report"
EMAIL_TYPE = "outlook"

def download_csi_emails():
    """Download CSI property emails using Gmail API"""
    try:
        print(f"🏨 Processing emails for CSI")
        ...
        if success:
            print(f"✅ Successfully processed CSI emails")
        else:
            print(f"⚠️ No CSI emails found")
            
        return success
        
    except Exception as e:
        print(f"❌ An error occurred processing CSI emails: {e}")
        return False

def main():
    """Main execution function"""
    print("CSI Email Download Script - Gmail API Version")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*60)
    
    success = download_csi_emails()
    
    print("="*60) 
    if success:
        print("✅ CSI email processing completed successfully")
    else:
        print("❌ CSI email processing failed or found no emails")
