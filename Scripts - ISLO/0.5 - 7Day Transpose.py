import openpyxl  # Variable items = ISLO (Property nickname)
...
# Construct the folder path for the 7 days ago extract, including the 'ISLO' subfolder
source_folder = os.path.join(base_source_folder, f"Extract {seven_days_ago.strftime('%Y-%m-%d')}", "ISLO")
...
if not source_file:
    print(f"No source file found for {eight_days_ago.strftime('%d-%b-%Y')} in the {seven_days_ago.strftime('%Y-%m-%d')} 'ISLO' subfolder.")
else:
    try:
