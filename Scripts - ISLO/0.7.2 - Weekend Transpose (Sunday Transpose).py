...
# Construct the folder path for today's extract, including the 'ISLO' subfolder
source_folder = os.path.join(base_source_folder, f"Extract {today.strftime('%Y-%m-%d')}", "ISLO")

# Calculate the date one day prior for the file name
previous_day = today - timedelta(days=1)

# Find the source file in the 'ISLO' subfolder
for file in os.listdir(source_folder):
    if file.endswith('.xlsx') and previous_day.strftime('%d-%b-%Y') in file:
        source_file = os.path.join(source_folder, file)
        break
else:
    print(f"No source file found for {previous_day.strftime('%d-%b-%Y')} in today's 'ISLO' subfolder.")
    exit()
