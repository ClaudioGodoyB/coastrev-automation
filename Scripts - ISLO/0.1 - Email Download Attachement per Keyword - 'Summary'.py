import os #Variable items = ISLO (Property nickname), 92817 (ASI Property ID)
import win32com.client
import datetime
from datetime import timedelta

def download_attachments(subject_keyword, save_folder, folder_date):
    # Updated folder structure: "Extract YYYY-MM-DD\ISLO"
    folder_name = f"Extract {folder_date}\\ISLO"
            ...
            if message.SenderEmailAddress == 'noreply@anandsystems.email':  # Check if the sender matches
                if folder_date in message.ReceivedTime.strftime('%Y-%m-%d') and subject_keyword in message.Subject:
                    ...
                    for attachment in message.Attachments:
                        # Only process files containing '92817' in their name
                        if '92817' in attachment.FileName:
