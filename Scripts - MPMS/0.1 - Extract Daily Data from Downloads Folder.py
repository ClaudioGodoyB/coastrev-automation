import os #Variables = MPMS (Property Nickname), Booking analysis (file name snippet)
import shutil
...
def find_and_copy_file(downloads_folder, save_root_folder):
    csv_files = [f for f in os.listdir(downloads_folder)
                 if f.lower().endswith('.csv') and 'booking analysis' in f.lower()]
    ...
    destination_folder = os.path.join(save_root_folder, dated_folder, 'MPMS')
